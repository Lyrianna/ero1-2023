...
def is_matching(G, matching):
    ...
    matching : set
        Set representing a matching. It must have elements
    ...
    -------
    bool
        Whether the given set represents a valid matching in the graph.
...
def is_perfect_matching(G, matching):
    ...
    -------
    bool
        Whether the given set represents a valid perfect matching in the graph.
...
def min_weight_matching(G, maxcardinality=False):
    ...
    """
    if len(G.edges) == 0:
        return max_weight_matching(G, maxcardinality)
    min_weight = min(G.edges.values())
    inv_edges = ((u, v, 1 / (1 + w - min_weight)) for u, v, w in G.iter_edges())
    InvG = Graph(inv_edges)
    return max_weight_matching(InvG, maxcardinality)
        ...
        for bv in path:
            if isinstance(bv, Blossom):
                if bv.mybestedges is not None:
                    ...
                    nblist = [
                        (v, w) for v in bv.leaves() for w in G.neighbours(v) if v != w
                    ]
            else:
                nblist = [(bv, w) for w in G.neighbours(bv) if bv != w]
            for k in nblist:
                (i, j) = k
                if inblossom[j] == b:
                    i, j = j, i
